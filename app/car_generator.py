...
def _copy_apk(apk: StandardApkSpec, dst: Path, keep_paths: set[Path]) -> None:
    ...
    Бросает CarGenerationError, если apk.path физически отсутствует на диске
    — реальный инцидент (2026-09, Geely Atlas Обычная): standard_apks_optional
    из load_car_spec() указывает на files/pack*/optional/<файл>, который
    ЛЕНИВО докачивается только по кнопке "Скачать файлы модели" (см.
    content_sync.sync_model_files) — если техник открыл редактор на машине,
    где эта папка ещё не докачана, apk.path.resolve()==dst совпадает (тот же
    путь, ничего переносить не нужно), проверка на равенство путей ниже
    раньше молча пропускала shutil.copy2 БЕЗ проверки, что файл вообще
    существует — сохранение тихо "проглатывало" такой APK: он не копировался
    (нечего было копировать), но keep_paths.add(dst) всё равно срабатывал,
    а _render_spec_json ниже по стеку вызовов писал его в _wizard_spec.json,
    как будто всё в порядке. На сервере, где физически лежит только то, что
    реально было в архиве upload_model (см. car_editor_api.py), такой файл
    исчезал бесследно — вместе с записью о нём при следующей публикации,
    если что-то в цепочке МЕЖДУ такой публикацией и этой ещё расставило точки
    над i для остальных полей (см. car_editor_api.py:_worker). Явная ошибка
    здесь останавливает СОХРАНЕНИЕ до того, как install.py/stages.py/
    _wizard_spec.json будут перезаписаны (они пишутся последними в
    _write_model_files) — старая, ещё верная версия модели остаётся на
    месте, а не тихо портится."""
    dst = dst.resolve()
    same_location = apk.path.resolve() == dst
    if not same_location and not apk.path.is_file():
        raise CarGenerationError(
            f"Файл {apk.path.name} не найден на диске. Похоже, файлы этой модели ещё не "
            'докачаны полностью на этой машине — нажмите "Скачать файлы модели" и повторите.'
        )
    if same_location and not dst.is_file():
        raise CarGenerationError(
            f"Файл {dst.name} должен уже быть на месте, но отсутствует на диске. Похоже, файлы "
            'этой модели ещё не докачаны полностью на этой машине — нажмите "Скачать файлы '
            'модели" и повторите.'
        )
