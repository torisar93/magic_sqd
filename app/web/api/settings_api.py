...
def is_under_program_files(base_dir: Path) -> bool:
    ...
    самая частая причина отказов записи (см. set_debug_mode/sync_api): туда
    попадали установки «для всех пользователей» (до 1.0.7 мастер это позволял),
    а обычный пользователь без прав администратора имеет там только чтение. Нужно
