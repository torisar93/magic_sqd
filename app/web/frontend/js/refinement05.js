...
(() => {
  ...
  LabUI.connection=({port=5555,host='',title='Подключение по Wi-Fi',help='',scan,connect,onClose})=>{
    ...
    async function searchNow(){const p=validPort();if(!p||busy)return;const g=++generation;search.disabled=true;status.textContent='Ищем устройства в сети…';status.dataset.state='searching';results.replaceChildren();
      try{const hosts=await scan(p);if(g!==generation||!modal.open)return;status.textContent=hosts.length?'Выберите устройство для подключения':'Устройства не найдены. Можно ввести адрес вручную.';hosts.forEach(item=>{const h=typeof item==='string'?item:item.host;const recommended=typeof item==='object'&&item.recommended;const b=n('button','connection-device'+(recommended?' recommended':''));b.append(LabUI.symbol(recommended?'check':'wifi'),n('span','',h));if(recommended)b.append(n('small','connection-badge','Рекомендовано'));if(item.port)b.append(n('small','',String(item.port)));else if(!recommended)b.append(n('small','','В сети'));b.onclick=()=>{hostInput.value=h;if(item.port)portInput.value=item.port;results.querySelectorAll('button').forEach(x=>x.classList.toggle('selected',x===b));hostInput.focus();};results.append(b);});}
