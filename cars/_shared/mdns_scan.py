...
Используется из wifi_adb.py/telnet_adb.py (см. scan_network_for_wifi_adb) —
общий для обоих файл в той же cars/_shared/, а не дублирование в каждом:
дублируется код только на границе app/ <-> cars/_shared/ (см. их докстринги
про "подгружается отдельно, без доступа к app/"), а не между файлами внутри
самой cars/_shared/."""
