...
(() => {
  ...
  function detail({brand,group,src,heroSrc,versions,onOpen,selectedKey}){
    ...
    versions.forEach(v=>{const row=n('label','model-version');const input=n('input');input.type='radio';input.name='model-version';input.value=v.key;input.checked=v===chosen;const text=n('span','');text.append(n('strong','',v.modification||'Основная версия'));if(statusNames[v.status_color])text.append(n('small','version-state '+v.status_color,statusNames[v.status_color]));if(v.submission_status_label)text.append(n('small','version-state submission-'+v.submission_status,v.submission_status_label));const preview=image(v.logo||src,'','version-visual');row.append(input,preview,text);input.addEventListener('change',()=>{chosen=v;if(window.Catalog09?.transition){Catalog09.transition(hero,{kind:'model',brand,model:group,modification:v.modification||'',selectionKey:v.key,original:v.logo||src,heroSrc:v.hero||heroSrc||'',alt:group})}else{const next=makeHero();hero.replaceWith(next);hero=next}});choices.append(row)});
